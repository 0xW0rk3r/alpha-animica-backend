...
// Middleware
app.use(cors({
  origin: 'https://alpha-animica-gilt.vercel.app',
  credentials: true,
}));
...
function getActiveSubscription(userId, cb) {
  db.get(`
    ...
    LEFT JOIN subscription_plans p ON s.plan_type = p.name
...
function initializeDatabase() {
  db.serialize(() => {
    ...
    // Ensure Free Tier plan exists
    db.get('SELECT * FROM subscription_plans WHERE name = ?', ['Free Tier'], (err, plan) => {
      if (err) {
        console.error('Error checking for Free Tier plan:', err);
      } else if (!plan) {
        db.run('INSERT INTO subscription_plans (name, price, applications_limit, opportunities_limit, duration_days, description) VALUES (?, ?, ?, ?, ?, ?)',
          ['Free Tier', 0.0, 3, 3, 30, 'free tier plan']);
      }
    });
      ...
      if (err) {
        ...
        bcrypt.hash(adminPassword, 10, (err, hash) => {
          if (err) {
            ...
            db.run('INSERT INTO users (email, password, name, user_type) VALUES (?, ?, ?, ?)', [adminEmail, hash, adminName, adminUserType], (err) => {
              if (err) {
  ...
  try {
    ...
    db.run(
      ...
      function(err) {
        ...
        // Assign Free Tier plan by default
        db.get('SELECT * FROM subscription_plans WHERE name = ?', ['Free Tier'], (err, plan) => {
          if (!err && plan) {
            const expiresAt = new Date(Date.now() + plan.duration_days * 24 * 60 * 60 * 1000).toISOString();
            db.run('INSERT INTO subscriptions (user_id, plan_type, applications_limit, opportunities_limit, expires_at) VALUES (?, ?, ?, ?, ?)',
              [userId, plan.name, plan.applications_limit, plan.opportunities_limit, expiresAt]);
          }
        });
        ...
        if (userType === 'trainee') {
          ...
          db.run(
            ...
            function(profileErr) {
              ...
              return res.json({ token, user: { id: userId, email, name, userType } });
            }
          );
...
app.post('/api/opportunities', authenticateToken, (req, res) => {
  ...
  getActiveSubscription(clinicId, (err, sub) => {
    ...
    if (sub.opportunities_limit <= 0) return res.status(403).json({ error: 'Opportunity limit reached for your subscription' });
    db.run(
      ...
      function(err2) {
        if (err2) return res.status(500).json({ error: 'Failed to create opportunity' });
...
app.post('/api/applications', authenticateToken, upload.single('resume'), (req, res) => {
  ...
  db.get('SELECT id FROM applications WHERE trainee_id = ? AND opportunity_id = ?', 
    ...
    // ENFORCE SUBSCRIPTION LIMIT for trainees
    getActiveSubscription(req.user.id, (err, sub) => {
      ...
      if (!sub) {
        ...
        db.get('SELECT COUNT(*) as cnt FROM applications WHERE trainee_id = ?', [req.user.id], (err2, countRes) => {
          ...
          if (countRes.cnt >= 3) return res.status(403).json({ error: 'You have reached your free application limit. To upgrade, contact the admin at alphaanimica@gmail.com.' });
          // Proceed to insert application
          db.run('INSERT INTO applications (trainee_id, opportunity_id, cover_letter, resume_path) VALUES (?, ?, ?, ?)', [req.user.id, opportunityId, coverLetter, resumePath], function(err3) {
            if (err3) return res.status(500).json({ error: 'Failed to submit application' });
            res.json({ id: this.lastID, message: 'Application submitted successfully (free tier)' });
          });
        });
        return;
      }
      if (sub.applications_limit <= 0) return res.status(403).json({ error: 'You have reached your application limit. To upgrade, contact the admin at alphaanimica@gmail.com.' });
      // Proceed to insert application and decrement limit
      db.run('INSERT INTO applications (trainee_id, opportunity_id, cover_letter, resume_path) VALUES (?, ?, ?, ?)', [req.user.id, opportunityId, coverLetter, resumePath], function(err4) {
        if (err4) return res.status(500).json({ error: 'Failed to submit application' });
...
app.delete('/api/opportunities/:id', authenticateToken, (req, res) => {
  ...
  db.run('DELETE FROM opportunities WHERE id = ? AND clinic_id = ?', [opportunityId, clinicId], function(err) {
    if (err) {
      ...
      return res.status(500).json({ error: 'Failed to delete opportunity', details: err.message });
    }
    if (this.changes === 0) return res.status(403).json({ error: 'Not authorized or opportunity not found' });
...
app.delete('/api/admin/users/:id', authenticateToken, (req, res) => {
  ...
  // Remove uploaded files for this user (resume, profile photo, logo, clinic photos)
  db.get('SELECT * FROM trainee_profiles WHERE user_id = ?', [userId], (err, traineeProfile) => {
    if (traineeProfile) {
      if (traineeProfile.resume_path) try { fs.unlinkSync(traineeProfile.resume_path); } catch {}
      if (traineeProfile.profile_photo) try { fs.unlinkSync(traineeProfile.profile_photo); } catch {}
    }
    db.get('SELECT * FROM clinic_profiles WHERE user_id = ?', [userId], (err, clinicProfile) => {
      if (clinicProfile && clinicProfile.logo_path) try { fs.unlinkSync(clinicProfile.logo_path); } catch {}
      db.all('SELECT * FROM clinic_photos WHERE clinic_id = ?', [userId], (err, photos) => {
        if (photos && photos.length) {
          photos.forEach(photo => { if (photo.photo_path) try { fs.unlinkSync(photo.photo_path); } catch {} });
        }
        // Now delete the user (cascades to all related data)
...
app.post('/api/admin/assign-subscription', authenticateToken, (req, res) => {
  ...
  db.get('SELECT * FROM subscription_plans WHERE id = ?', [plan_id], (err, plan) => {
    ...
    const expiresAt = new Date(Date.now() + plan.duration_days * 24 * 60 * 60 * 1000).toISOString();
    db.get('SELECT * FROM subscriptions WHERE user_id = ?', [user_id], (err, existing) => {
      if (err) {
        console.error('Assign subscription error:', err);
        return res.status(500).json({ error: 'Failed to check existing subscription', details: err.message });
      }
      if (existing) {
        db.run(
          'UPDATE subscriptions SET plan_type = ?, applications_limit = ?, opportunities_limit = ?, expires_at = ? WHERE id = ?',
          [plan.name, plan.applications_limit, plan.opportunities_limit, expiresAt, existing.id],
          function(err) {
            if (err) {
              console.error('Assign subscription update error:', err);
              return res.status(500).json({ error: 'Failed to update subscription', details: err.message });
            }
            res.json({ id: existing.id, message: 'Subscription updated' });
          }
        );
      } else {
        db.run(
          ...
          function(err) {
            if (err) {
              console.error('Assign subscription insert error:', err);
              return res.status(500).json({ error: 'Failed to assign subscription', details: err.message });
            }
